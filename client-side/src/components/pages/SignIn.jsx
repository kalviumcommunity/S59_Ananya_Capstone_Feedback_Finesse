...
function SignIn() {
  ...
  // console.log(isToggled)
   ...
   const handleLogin = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_URI}/register/login`, {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        if (message.role != role) {
          toast.error("Please log in with valid credentials for the specified role !")
        }
        else {
        ...
        <section id="signin">
          ...
          <form action="" onSubmit={handleSubmit(handleLogin)}>
            <div>
              ...
              className={errors.username ? "input-error" : ""} {...register("username", {
                required: "Enter your username please!"})} />
            </div>
            <div>
              <h5>Password</h5>
              <input type="password" placeholder="Enter your password" className={errors.password ? "input-error" : ""} {...register("password", {
            ...
            <h4 className="text-darkred">
              Not made an account yet?  <Link to={'/signup'}>
              <u>Register</u> 
              </Link> from here
            </h4>
